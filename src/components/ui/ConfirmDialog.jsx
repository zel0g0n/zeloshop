...
const ConfirmDialog = ({ title, message, confirmLabel = "Ha", cancelLabel = "Bekor qilish", danger, busy, onConfirm, onCancel }) => {
  // `busy` paytida (tasdiqlash/bekor qilish so'rovi allaqachon
  // yuborilgan) Escape ham o'chiriladi - xuddi pastdagi tugmalar
  // `disabled={busy}` bo'lgani kabi, ikkilanib qolgan holatni oldini
  // olish uchun.
  useEscapeToClose(onCancel, !busy);

  return (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4" role="alertdialog" aria-modal="true">
