...
import { ArrowLeft, Search, ImageOff, Check, X, Loader2, Flame, Timer } from "lucide-react";
...
/**
 * "AKSIYA YARATISH" — mahsulotga TO'G'RIDAN-TO'G'RI chegirma narxi
 * qo'yish sahifasi.
 *
 ...
 * bo'limida avtomatik ko'rinadi. Promokod (Marketing sahifasida) esa
...
const CreatePromotionPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F9] dark:bg-slate-950 text-slate-900 dark:text-white pb-36 transition-colors duration-300">
      <div className="sticky top-0 z-30 bg-white/95 dark:bg-slate-900/95 px-4 py-4 shadow-xs">
        <div className="flex items-center gap-3 mb-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 active:scale-95 transition-transform"
          >
            <ArrowLeft size={17} />
          </button>
          <div className="min-w-0">
            <h1 className="text-sm font-black text-slate-800 dark:text-white truncate">{t("createPromotion.pageTitle")}</h1>
            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider truncate">{t("createPromotion.subtitle")}</p>
          </div>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-3 bg-[#F4F5F9] dark:bg-slate-800 rounded-xl text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-500"
          />
        </div>
      </div>
