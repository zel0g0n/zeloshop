...
const SearchOverlay = ({ onClose }) => {
  ...
  const { handleSearchChange } = useLiveSearch();
  const { results, isLoading } = useProductSearchSuggestions(queryValue);
  ...
  return (
    <div className="fixed inset-0 bg-white z-[100] max-w-[440px] mx-auto flex flex-col animate-fadeIn">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {queryValue && !isLoading && results.length > 0 && (
          <div>
            ...
            <ul className="divide-y divide-gray-50">
              {results.map((item) => (
                <li 
                  key={item.id}
                  onClick={() => goToCatalogWith(item.name)}
